  ...
  const handleApproximation = () => {
    if (!process.env.NODE_ENV || process.env.NODE_ENV === 'development') {
      ...
      approximationSrv.fitCurves(request, approxMetadata, (err, res) => {
        ...
        dispatch({type: FitActionType.SET_RESULT, result: res.toObject().fitResultList});
      });
      graphDataManager.clearExpressions();
    }
  };
    ...
    <>
      ...
      <div className={styles.curveFitWrapper}>
        ...
        <div className={styles.fittingButtons}>
          ...
          <button className={styles.button} type="button" onClick={toggleDataModal}>Enter data</button>
          <p>
            2. Select models for fitting,
            add temporary models
            (will be lost on page reload),
            set parameters initial guess and bounds values
            (Initial guess value is always 1, bounds are -Infinite, Infinite)
          </p>
          <button className={styles.button} type="button" onClick={toggleModelsSelection}>Select models</button>
          <p>3. Send fit request</p>
          <button className={styles.button} type="button" onClick={handleApproximation}>Fit</button>
        </div>
